...
function NavIcon({ name, active }: { name: NavIconName; active?: boolean }) {
  const stroke = active ? "currentColor" : "rgba(233,234,229,0.92)";
  const common = {
    fill: "none",
    stroke,
    strokeWidth: 1.8,
  ...
  return (
    <div className="min-h-screen text-bone-100">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(rgba(213,255,64,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(192,194,184,0.04) 1px, transparent 1px)",
      ...
      <div className="relative min-h-screen lg:grid lg:grid-cols-[19rem_minmax(0,1fr)]">
        ...
        <aside
          className={`fixed inset-y-0 left-0 z-50 flex w-[18.5rem] flex-col border-r border-white/10 bg-ink-600/96 px-5 py-5 shadow-2xl backdrop-blur-xl transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${
            navOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-none bg-lime-300 text-[1.65rem] font-black leading-none text-ink-900 shadow-glow-lime">
                λ
              </div>
              <div>
          ...
          <nav className="mt-6 flex flex-1 flex-col gap-1" aria-label="Primary">
            {nav.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  [
                    "group rounded-none border px-3.5 py-3 text-left",
                    isActive
                      ? "border-lime-300/35 bg-lime-300/10 text-lime-100 shadow-[0_12px_30px_-22px_rgba(213,255,64,0.55)]"
                      : "border-transparent text-bone-300 hover:border-white/10 hover:bg-ink-300/70 hover:text-bone-50",
                  ].join(" ")
                }
              >
                {({ isActive }) => (
                  <div className="flex items-center justify-between gap-3">
                      ...
                      <div
                        className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-none border ${
                          isActive
                            ...
                            : "border-white/10 bg-ink-300/72 text-bone-200"
                        }`}
                      >
                    ...
                    {item.badge && (
                      <span
                        className={`shrink-0 rounded-none px-2 py-1 text-[10px] font-bold tracking-[0.16em] ${
                          item.badgeClass ?? "bg-ink-200/80 text-bone-300"
                        }`}
                      >
                        {item.badge}
                      </span>
